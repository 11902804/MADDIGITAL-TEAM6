...
def on_message(message):
    ...
    if topic_ai == 1 and topic_team4 == 1:
        topic_ai = 0
        topic_team4 = 0
        afstandPersoon0 = math.sqrt((int(persoon0X) - data["stengels"]["beweegbare"][0]["X"]) ** 2 + (
                int(persoon0Y) - data["stengels"]["beweegbare"][0]["Y"]) ** 2)
        ...
        client.publish("0,1", "{%s,%s,[%s],[%s],[%s],[%s,%s]}" % (
            mode, freq, persoon0RGB1, persoon0RGB2, persoon0RGB3, hoekjeAlpha,
            hoekjeTheta))
